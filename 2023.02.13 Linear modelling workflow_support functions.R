...
#-#########################################-#
# Step plot settings: Basic ggplot themes  ----
#-#########################################-#
...
#-###################################################-#
# Step 1: Import dataset and specify variables ----
#-###################################################-#
...
#================================#
#* 2.2 Predictor collinearity ----
#================================#

#----------------------------------#
#** 2.2.1: Graphical inspection ----
#----------------------------------#
...
plot <-  lapply(levels(as.factor(dat_plot$pair_key)), 
                function(pair_key_value)  {
                  ggplot(subset(dat_plot, pair_key == pair_key_value), 
                    ...
                    geom_quasirandom(method = "smiley", groupOnX = TRUE) +
...
#-----------------------------------------------#
#** 2.2.2: Variance Inflation Factors (VIFs) ----
#-----------------------------------------------#
...
#==========================================#
#* 2.3 Predictor-response relationships ----
#==========================================#
...
#-################################-#
# Step 3: The statistical model ----
#-################################-#

#===============================================#
#* 3.1 Standardisation of numeric predictors ----
#===============================================#
...
#----------------------------------------#
#** 4.1.1: Distribution of residuals -----
#----------------------------------------#
...
residual_plots <- function(data, modelTMB, response) {
  ...
  plot2 <- list(ggplot(data = residuals, aes(x = mod.fit.rank, y = mod.res)) +
                  ...
                  geom_quasirandom(method = "smiley", groupOnX = TRUE, alpha = 0.6) +
                  theme(aspect.ratio = 1) +
                  plot0
  )
  ...
  if(length(mod$sdr$diag.cov.random) != 0) { # Needed to check for random component. Very circumstantial - is there an easier solution? ----
...
#---------------------------------------------------#
#** 4.1.2: Residuals against possible PREDICTORS ----
#---------------------------------------------------#
...
#------------------------------------------------------------------------------#  
#** 4.1.3: Residuals against possible TWO-WAY INTERACTIONS AMONG PREDICTORS ----
#------------------------------------------------------------------------------#   
  ...
  if(length(na.omit(predictors)) < 2) {stop("ERROR: At least two predictors are required")} else {
    ...
    plot3 <- lapply(levels(dat_plot$pair_key), function(pair_key_value)  {
      ...
      ggplot(dat_plot, aes(x = y, y = mod.res, color = x)) + 
        ...
        geom_quasirandom(method = "smiley", groupOnX = TRUE, alpha = 0.6, dodge.width = 0.8) +
        labs(y = "Standardised residuals", x = sub('\\:.*', '', pair_key_value), title = pair_key_value) +
        guides(color = guide_legend(title = sub('.*: ', '', pair_key_value))) +
        theme(legend.justification = c(0.02, 0.98), legend.position = c(0.02, 0.98), legend.key.size = unit(0.2, 'cm')) + 
...
#------------------------------------------------------------------------#
#** 4.1.4: Residuals against PREDICTORS split by RANDOM FACTOR LEVELS ----
#------------------------------------------------------------------------#
...
residual_plots_random_slopes <- function(data, modelTMB, fixed_eff, random_eff) {
  ...
  lapply(random_eff, function(random_eff_value){
    ...
    if(length(num) > 0) {plot1 <- lapply(num, function(x_value){
      ggplot(data, aes_(x = as.name(x_value), y = mod.res)) + 
        scale_y_continuous(limits = c(0,1)) +
        geom_point() +
    ...
    if(length(fac) > 0) {plot2 <- lapply(fac, function(x_value){
      ggplot(data, aes_(x = as.name(x_value), y = mod.res)) + 
        scale_y_continuous(limits = c(0,1)) +
        geom_boxplot(outlier.shape = NA) +
        geom_quasirandom(method = "smiley", groupOnX = TRUE, alpha = 0.6, col = "grey40", cex = 0.7) +
        facet_wrap(~ data[,random_eff_value]) + 
        labs(y = "Standardised residuals", 
             x = as.name(x_value), 
             title = paste("Std residuals as a function of", x_value, "split by", random_eff_value, collapse = " ")) +
...
dispersion_simulation <- function(data, modelTMB, response, predictor, n.sim) {
  ...
  plot1 <- ggplot() + 
    ...
    geom_vline(data = temp, aes(xintercept = variance), col = col.vline_variance, size = 1.5) +
    plot0 +
...
zero_simulation <- function(data, modelTMB, response, predictor, n.sim) {
  ...
  plot.output <- ggplot() +
    ...
    geom_vline(data = temp, aes(xintercept = zeros), col = col.vline, size = 1.2, lty = 1) +
    scale_x_continuous() +
    plot0
  if(!missing(predictor) && !is.null(predictor) && length(na.omit(predictor)) > 0) {
...
#-################################################-#
# Step 5: Model results and parameter estimates ----
#-################################################-#

#====================================#
#* 5.1 Overall coefficient estimates  
#====================================#
...
#=========================================#
#* 5.2 Estimation of regression slopes ---- 
#=========================================#

slope_estimates_0 <- function(data, modelTMB, num_predictor, z.invert, fac_predictor, n.sim) {
  ...
  if (missing(fac_predictor)) {
    stop("ERROR: No factor predictor has been specified. Skip this step if none is available.")}
  
  if (missing(z.invert)) {z.invert = F}
  
  if(length(fac_predictor) > 2) {
    stop("ERROR: You specified more than two factor predictors. This function only works up to three way interactions.")}
  ...
  int_check <- sapply(interactions,str_detect,fac_predictor, simplify = F)
  int_check <- names(int_check)[str_detect(names(int_check), num_predictor)]
  
  if(sum(str_detect(int_check, fac_predictor)) != length(fac_predictor)) {
  ...
  poly_form <- fixed.pred[str_detect(fixed.pred, pattern = "poly")] # find poly formula if present
  poly_term <- poly_form[str_detect(poly_form, pattern = num_predictor)]
  
  if(length(poly_term) > 0) {
    stop("ERROR: Your model contains a polynomial term. Slopes computation is not meaningful.")}
  
  if(sum(str_detect(fixed.pred, pattern = fixed(num_predictor))) < length(num_predictor)) {
    stop("ERROR: Incorrect assignment of predictors. Specify NUMERIC predictors that are part of the model formula only. Skip this step if not available.")}
  
  bsim <- simulate_model(modelTMB, iterations = n.sim, component = "cond") # change to "cond" only? 
  
  # We restrict bsim to columns that contain slope information:
  bsim <- bsim %>% 
    dplyr::select(contains(num_predictor)) 
  
  # Now we define the combinations of factor levels for which we need slope information: 
  fixed.pred.2 <- unique(subset(fixed.pred, fixed.pred %in% fac_predictor))
  
  # Now we start looping through all possible combinations of factor levels, and extract their coefficient estimates from bsim:
  fac1.lev <- length(levels(data[, fixed.pred.2[1]]))
  
  if(length(fixed.pred.2) > 1){
    fac2.lev <- length(levels(data[, fixed.pred.2[2]]))
  }
  
  bsim2 <- data.frame(matrix(NA, nrow = nrow(bsim)))

  if(length(fixed.pred.2) == 1) {
    for (i in 1:fac1.lev) {
      dat.value <-  bsim[, 1]  # The baseline covariate slope, always in column 1
      if (i > 1) {
        # adds the extra information for further levels of factor 1 (except for first level).
        dat.value <- dat.value + bsim[, i] 
      }
      bsim2[, levels(data[, fixed.pred.2[1]])[i]]  <- dat.value
    }
  } else {
    for (i in 1:fac1.lev) {
      for (j in 1:fac2.lev) {
        ...
        if (j > 1) {
          # adds the extra information for further levels of factor 2 (except for first level).
          dat.value <- dat.value + bsim[, (fac1.lev + (j - 1))] 
        }
        if (i > 1 & j > 1 & 
            length(bsim) > (fac1.lev + fac2.lev - 1)) {
          # all these cases require further addition of interaction terms.
          # Note that these are only present if the model contained the 3-way interaction.
          dat.value <- dat.value + bsim[, (fac1.lev + fac2.lev - 1 + (i-1) + (j-2)*(fac1.lev-1))]
        }
        # Now we enter this value in bsim:
        bsim2[, paste(levels(data[, fixed.pred.2[1]])[i], 
                      levels(data[, fixed.pred.2[2]])[j], 
                      sep = " : ")]  <- dat.value
      }
    }}
  bsim <- bsim2[,-1]

  if(!is.na(fac_predictor[1])) {
    
    if(ncol(bsim) == 1) {
      bsim %>% pivot_longer(everything()) %>%  
        dplyr::rename(slope = name) -> bsim
      stop("HINT: Your numeric predictor is not part of an interaction. Hence, the function provides a single, overall, slope estimate.")
    }
    
    if(sum(sapply(data[fac_predictor], FUN = is.factor)) != length(fac_predictor)) {
      stop("ERROR: Incorrect assignment of factor predictors. Specify FACTOR predictors only.")}
  ...
  # Transform to covariate raw scale from z-scale.
  if(z.invert & grepl("_z", num_predictor)) {
    bsim$value <- bsim$value / sd(data[, gsub("_z", "", num_predictor) ])
    num_predictor <- gsub("_z", "", num_predictor) # for correct x-axis label.
  }
  
    if(modelTMB$modelInfo$family$link %in% c("log", "logit")) bsim %>% mutate(value = exp(value)) -> bsim 
    # back-transformation to odds ratios (for logit-link models), or to ratios (for log-link models).
  
  bsim %>% 
    ggplot(aes(x = value, y = slope)) +
    {if(modelTMB$modelInfo$family$link %in% c("log", "logit")) 
    ...
    stat_pointinterval(.width = .95, size = 2.5) +
    {if(modelTMB$modelInfo$family$link == "log") 
      labs(x = paste("Slope for ", num_predictor, " [response ratio per unit increase]", sep = ""),
           y = NULL)} +
    {if(modelTMB$modelInfo$family$link == "logit") 
      labs(x =  paste("Slope for ", num_predictor, " [response odds ratio per unit increase]", sep = ""),
           y = NULL)} +
    {if(modelTMB$modelInfo$family$link == "identity") 
      labs(x = paste("Slope for ", num_predictor, " [absolute response change per unit increase]", sep = ""),
           y = NULL)} +
    {if(!(modelTMB$modelInfo$family$link %in% c("identity", "log", "logit"))) 
      labs(x = paste("Slope for ", num_predictor, " on link scale: ", modelTMB$modelInfo$family$link, sep = ""),
           y = NULL)} +
  ...
  as.data.frame(bsim  %>%  group_by(slope) %>% 
                  summarise(median = median(value), 
                            lower = quantile(value, prob = 0.025), 
                            upper = quantile(value, prob = 0.975),
                            prop_change_median = median - 1,
                            prop_change_lower = lower - 1, 
                            prop_change_upper = upper - 1))
}
...
plot.output <- plot.output + 
  {if(modelTMB$modelInfo$family$link == "identity") 
    labs(x = paste(num_predictor, "slope [absolute response change +/- 95% CI]", sep = " "),
         y = NULL)} +
{if(modelTMB$modelInfo$family$link == "log") 
  labs(x = paste(num_predictor, "slope [proportional change +/- 95% CI]", sep = " "),
       y = NULL)} +
  {if(modelTMB$modelInfo$family$link == "logit") 
    labs(x =  paste(num_predictor, "slope [proportional change +/- 95% CI]", sep = " "),
         y = NULL)} +
  {if(!(modelTMB$modelInfo$family$link %in% c("identity", "log", "logit"))) 
    labs(x = paste(num_predictor, "slope [link scale response change +/- 95% CI]",sep = " "),
         y = NULL)} +
  theme(axis.ticks.y = element_blank()) + 
  plot0 
...
#=========================================================#
#* 5.3 Estimation of differences between factor levels ---- 
#=========================================================#
...
#  based on object’s response transformation or link function.
  ...
  # 2023.02.07 - adjusted the following 10 lines to capture all possible combinations ----
  ...
  plot.output <- ggplot(comparisons, aes(x = !!sym(colnames(comparisons)[2]), y = contrast)) + # plots contrasts only with 95% compatibility interval
    geom_point() +
  #  scale_x_continuous(limits = c(0, max(comparisons$upper.CI))) +
    geom_errorbarh(aes(xmax = upper.CI, 
                       xmin = lower.CI, 
                       height = 0)) +
    theme(axis.ticks.y = element_blank()) +
  ...
  if (colnames(comparisons)[2] == "estimate" & modelTMB$modelInfo$family$link == "identity") {
    plot.output <- plot.output + geom_vline(xintercept = 0, linetype = 2, col = "blue") +
      labs(x = "Pairwise absolute differences: mean +/- 95% CI",
           y = NULL)}
  if (colnames(comparisons)[2] == "estimate" & modelTMB$modelInfo$family$link != "identity") {
    plot.output <- plot.output + geom_vline(xintercept = 0, linetype = 2, col = "blue") +
      labs(x = "Pairwise differences on link scale: mean +/- 95% CI",
           y = NULL)}
  if (colnames(comparisons)[2] == "ratio") {
    plot.output <- plot.output + geom_vline(xintercept = 1, linetype = 2, col = "blue") +
      labs(x = "Pairwise ratios: mean +/- 95% CI",
           y = NULL)}
...
#-#####################################################-#
# Step 6: Graphical display of the model predictions ----
#-#####################################################-#

#==================================#
#* 6.2 Derive model predictions ---- 
#==================================#
    ...
    if(length(predictors) == 2) {
      ...
      plot.2pred <- ggplot(data = predictions,
                           aes(x = as.numeric(predictions[, pred.x1]),
                               y = pred_mod), 
                           group = predictions[, pred.x2]) # CORRECTED 2023.02.06 - was WITHIN aesthetics, which causes trouble with 2 numerics ----
  ...
  #                   col = col.outline.points, alpha = alpha.points)
      ...
      if(!is.factor(data_summary[, pred.x1])) {
        ...
        plot.2pred <- plot.2pred +
          geom_point(data = data_summary, 
                     aes(x = as.numeric(data_summary[, pred.x1]), 
                         y = mean),    # 2023.02.06 - removed shape and fill by pred.x2 - not possible for numeric pred.x2!! ----
                     size = size.points, 
                     position = position_jitter(width = jitter.points, height = 0),   
                     col = col.outline.points, 
                     alpha = alpha.points)
